...
class ProspekCtaImport implements ToCollection, WithHeadingRow
{
    ...
    public $failures = [];
    public $updatedProspekCount = 0;
    public $updatedCtaCount = 0;
    public $skippedEmptyRowsCount = 0;
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $index => $row) {
            $rowNum = $index + 2; // Baris Excel dimulai dari 2 karena baris 1 adalah header
 
            // Jika kolom perusahaan kosong, lewati baris ini
            if (empty($row['perusahaan'])) {
                $this->skippedEmptyRowsCount++;
                continue;
            }
            ...
            try {
                ...
                // Pencocokan berbasis Perusahaan, Lokasi, dan Tanggal Prospek.
                // Jika tanggal berbeda, maka dianggap sebagai entri prospek baru (opportunity baru).
                $prospekQuery = Prospek::where('perusahaan', 'LIKE', '%' . $perusahaan . '%')
                    ->where('tanggal_prospek', $tanggalProspek);
                if (!empty($lokasi)) {
                ...
                $this->failures[] = [
                    'row' => $rowNum,
                    'perusahaan' => $row['perusahaan'] ?? 'Tidak Diketahui',
                    'error' => $e->getMessage()
                ];
            }
        }
