        ...
        <div class="row">
            <div class="col-12 col-xl-9 mx-auto">
                ...
                <div class="d-flex align-items-center mb-4">
                    <a href="{{ route('operational.permintaan-visual.biasa') }}" class="btn btn-light rounded-circle me-3 shadow-sm hover-lift border-0" style="width: 45px; height: 45px; display: flex; align-items: center; justify-content: center; background: white;">
                ...
                <div class="glass-card p-4 p-md-5">
                    <form action="{{ route('operational.permintaan-visual.biasa.update', $permintaan->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-4">
                            <div class="col-12">
                                ...
                                <input type="text" class="form-control-modern w-100" name="judul" value="{{ $permintaan->judul }}" required>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select-modern w-100" name="kategori" required>
                                    ...
                                    <option value="Cover Proposal" {{ $permintaan->kategori == 'Cover Proposal' ? 'selected' : '' }}>Cover Proposal</option>
                                    <option value="Flyer/Poster" {{ $permintaan->kategori == 'Flyer/Poster' ? 'selected' : '' }}>Flyer/Poster</option>
                                    <option value="Penjualan" {{ $permintaan->kategori == 'Penjualan' ? 'selected' : '' }}>Penjualan</option>
                                    <option value="Media Sosial" {{ $permintaan->kategori == 'Media Sosial' ? 'selected' : '' }}>Media Sosial</option>
                                    <option value="Presentasi" {{ $permintaan->kategori == 'Presentasi' ? 'selected' : '' }}>Presentasi</option>
                                    <option value="Lainnya" {{ str_contains(strtolower($permintaan->kategori), 'lain') ? 'selected' : '' }}>Lainnya</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label-modern">Target Selesai (Deadline) <span class="text-danger">*</span></label>
                                <input type="date" class="form-control-modern w-100" name="deadline" value="{{ \Carbon\Carbon::parse($permintaan->deadline)->format('Y-m-d') }}" required>
                            </div>

                            <div class="col-12">
                                <label class="form-label-modern">Tujuan / Kegunaan <span class="text-danger">*</span></label>
                                <input type="text" class="form-control-modern w-100" name="tujuan" value="{{ $permintaan->tujuan }}" required>
                            </div>

                            <div class="col-12">
                                <label class="form-label-modern">Deskripsi Kebutuhan Secara Detail <span class="text-danger">*</span></label>
                                <textarea class="form-control-modern w-100" name="deskripsi" rows="6" required>{{ $permintaan->deskripsi }}</textarea>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <div class="mb-3 d-flex align-items-center p-2 rounded border bg-light">
                                    <i class="fas fa-file-alt text-primary me-2"></i> File sebelumnya: {{ basename($permintaan->referensi_file) }}
                                </div>
                                @endif
                        ...
                        <div class="d-flex flex-column flex-md-row justify-content-end gap-3">
                            <a href="{{ route('operational.permintaan-visual.biasa') }}" class="btn-light-modern text-center text-decoration-none">Batalkan</a>
