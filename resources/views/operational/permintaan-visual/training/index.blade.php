            ...
            <div class="col-6 col-lg-3">
                <div class="glass-card p-4 d-flex align-items-center h-100">
                    ...
                    <div>
                        ...
                        <h3 class="fw-black text-dark mb-0" style="font-size: 28px;">{{ $statTotal }}</h3>
                    </div>
                </div>
            </div>
            <div class="col-6 col-lg-3">
                <div class="glass-card p-4 d-flex align-items-center h-100">
                    ...
                    <div>
                        ...
                        <div class="d-flex align-items-end gap-2">
                            <h3 class="fw-black text-dark mb-0" style="font-size: 28px;">{{ $statTotal - $statProses - $statSelesai }}</h3>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <button class="btn btn-outline-premium" data-bs-toggle="modal" data-bs-target="#modalDetailBerkas{{ $pelatihan->id }}">
...
@endphp
<div class="modal fade" id="modalDetailBerkas{{ $pelatihan->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content modal-content-modern">
            ...
            <div class="modal-body p-4 p-md-5" style="background-color: #f8f9fc;">
                <form action="{{ route('operational.permintaan-visual.training.upload', $pelatihan->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="d-flex justify-content-end mb-4">
                        <button type="submit" class="btn btn-premium px-4"><i class="fas fa-save me-2"></i> Simpan Perubahan</button>
                    </div>
                    <div class="row g-4">
                        @foreach($berkasList as $inputName => $berkas)
                        @php
                            ...
                            <div class="file-item-card h-100 d-flex flex-column">
                                    ...
                                    @if($hasFile)
                                        <div class="position-relative rounded-3 overflow-hidden mb-3 border shadow-sm">
                                            @if(in_array(strtolower($ext), ['jpg', 'jpeg', 'png', 'webp']))
                                                <img src="{{ Storage::url($filePath) }}" alt="Thumbnail" class="w-100 object-fit-cover" style="height: 140px;">
                                            @else
                                            ...
                                            <div class="position-absolute bottom-0 w-100 p-2 d-flex justify-content-between align-items-end" style="background: linear-gradient(to top, rgba(0,0,0,0.8), transparent);">
                                                <div class="text-white overflow-hidden text-truncate pe-2">
                                                    <div class="fw-bold text-truncate" style="font-size: 12px;">{{ basename($filePath) }}</div>
                                                </div>
                                                <a href="{{ Storage::url($filePath) }}" target="_blank" class="btn btn-sm btn-light text-primary rounded-circle shadow-sm flex-shrink-0 d-flex align-items-center justify-content-center" style="width: 32px; height: 32px;" title="Unduh">
                                ...
                                <div class="mt-auto border-top pt-3">
                                    <div class="input-group input-group-sm mb-2 rounded-3 overflow-hidden shadow-sm border" style="background: white;">
                                        <input type="file" class="form-control form-control-sm border-0 py-2 px-3 bg-white" name="{{ $inputName }}">
                                    </div>
                                </div>
                                @endif
                            </div>
