    ...
    <nav class="bg-white/80 backdrop-blur-md shadow-sm py-4 sticky top-0 z-30 border-b border-gray-100">
        <div class="max-w-3xl mx-auto px-5 flex items-center justify-between">
            <div class="flex items-center">
                <a href="<?php echo e(url('portal')); ?>" class="mr-4 p-2 bg-gray-50 rounded-full text-gray-500 hover:text-blue-600 hover:bg-blue-50 transition-colors active:scale-95">
    ...
    <main class="px-5 py-8 max-w-3xl mx-auto min-h-screen">
        ...
        <?php if(session('error')): ?>
            <div class="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-2xl mb-6 fade-in">
                ...
                <span class="block sm:inline text-sm"><?php echo e(session('error')); ?></span>
            </div>
        <?php endif; ?>
        ...
        <div id="search-section" class="fade-in block">
            ...
            <div class="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100">
                <form action="<?php echo e(route('portal.cek-status')); ?>" method="GET">
                    ...
                    <div class="space-y-4 mb-8">
                        ...
                        <div class="input-focus-ring transition-shadow rounded-xl">
                            <input type="text" name="id_pendaftaran" value="<?php echo e(request('id_pendaftaran')); ?>" class="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 outline-none transition-colors" placeholder="Contoh: IND-2026-001">
                        </div>
                    </div>
                    ...
                    <div class="space-y-5 mb-8">
                        ...
                        <div class="input-focus-ring transition-shadow rounded-xl">
                            <input type="text" name="nama_lengkap" value="<?php echo e(request('nama_lengkap')); ?>" class="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 outline-none transition-colors" placeholder="Nama Lengkap Sesuai KTP">
                        </div>
                        <div class="input-focus-ring transition-shadow rounded-xl">
                            <input type="date" name="tanggal_lahir" value="<?php echo e(request('tanggal_lahir')); ?>" class="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 outline-none transition-colors text-gray-700">
                        </div>
                    </div>
        ...
        <div id="result-section" class="fade-in block">
            ...
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 mb-6">
                <div class="flex justify-between items-start mb-6">
                    <div>
                        <h2 class="text-xl font-bold text-gray-900">Halo, <?php echo e($pendaftaran->nama_lengkap); ?></h2>
                        <p class="text-sm text-gray-500 mt-1">ID: <span class="font-bold text-gray-700"><?php echo e($pendaftaran->id_pendaftaran); ?></span></p>
                        <p class="text-sm text-gray-500">Program: <?php echo e($pendaftaran->training->nama_training ?? '-'); ?></p>
                    ...
                    <a href="<?php echo e(route('portal.cek-status')); ?>" class="text-xs font-bold text-blue-600 bg-blue-50 px-3 py-2 rounded-lg hover:bg-blue-100 active:scale-95 transition">
                        Cari Lain
                    </a>
                </div>
                ...
                <div class="flex flex-wrap gap-2 pt-4 border-t border-gray-50">
                    ...
                    <?php if(in_array($pendaftaran->status, ['diterima', 'revisi'])): ?>
                        <?php if($pendaftaran->pelatihanBerjalan && $pendaftaran->pelatihanBerjalan->link_zoom_pelatihan): ?>
                        <a href="<?php echo e($pendaftaran->pelatihanBerjalan->link_zoom_pelatihan); ?>" target="_blank" class="inline-flex items-center px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 border border-blue-100 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-colors active:scale-95 shadow-sm">
                            <i class="fas fa-video mr-2"></i> Zoom Pelatihan
                        </a>
                        <?php endif; ?>
                        <?php if($pendaftaran->pelatihanBerjalan && $pendaftaran->pelatihanBerjalan->link_zoom_asesmen): ?>
                        <a href="<?php echo e($pendaftaran->pelatihanBerjalan->link_zoom_asesmen); ?>" target="_blank" class="inline-flex items-center px-4 py-2 bg-purple-50 hover:bg-purple-100 text-purple-600 border border-purple-100 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-colors active:scale-95 shadow-sm">
                            <i class="fas fa-video mr-2"></i> Zoom Asesmen
                        </a>
                        <?php endif; ?>
                        <?php if($pendaftaran->pelatihanBerjalan && $pendaftaran->pelatihanBerjalan->modul): ?>
                            <?php if($pendaftaran->is_modul_downloaded): ?>
                            ...
                            <?php else: ?>
                            <a href="<?php echo e(route('portal.download-modul', $pendaftaran->id)); ?>" onclick="return confirm('PERINGATAN!\n\nSesuai dengan SOP dan Hak Cipta, Modul Materi untuk Peserta HANYA DAPAT DIUNDUH 1 KALI.\n\nApakah Anda yakin ingin mengunduhnya sekarang?');" class="inline-flex items-center px-4 py-2 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-colors active:scale-95 shadow-sm">
                                <i class="fas fa-book mr-2"></i> Unduh Modul
                            </a>
                            <?php endif; ?>
                        <?php endif; ?>
                        <?php if($pendaftaran->pelatihanBerjalan && $pendaftaran->pelatihanBerjalan->rundown_pelatihan): ?>
                        <a href="<?php echo e(asset($pendaftaran->pelatihanBerjalan->rundown_pelatihan)); ?>" target="_blank" class="inline-flex items-center px-4 py-2 bg-green-50 hover:bg-green-100 text-green-700 border border-green-200 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-colors active:scale-95 shadow-sm">
                            <i class="fas fa-list mr-2"></i> Rundown Acara
                        </a>
                        <?php endif; ?>
                        <?php if($pendaftaran->pelatihanBerjalan && $pendaftaran->pelatihanBerjalan->background_zoom): ?>
                        <a href="<?php echo e(asset($pendaftaran->pelatihanBerjalan->background_zoom)); ?>" target="_blank" class="inline-flex items-center px-4 py-2 bg-teal-50 hover:bg-teal-100 text-teal-700 border border-teal-200 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-colors active:scale-95 shadow-sm">
                            <i class="fas fa-image mr-2"></i> Background Zoom
                        </a>
                        <?php endif; ?>
                        <?php if(!$pendaftaran->pelatihanBerjalan || (!$pendaftaran->pelatihanBerjalan->link_zoom_pelatihan && !$pendaftaran->pelatihanBerjalan->link_zoom_asesmen && !$pendaftaran->pelatihanBerjalan->modul)): ?>
                    ...
                    <div class="bg-blue-50/50 p-3.5 rounded-xl border border-blue-100 flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-bold text-gray-800"><?php echo e($marketingName); ?></p>
                            </div>
                        </div>
                        <?php if($marketingWa != '#'): ?>
                        <a href="<?php echo e($marketingWa); ?>" target="_blank" class="w-8 h-8 bg-green-500 text-white rounded-full flex items-center justify-center hover:bg-green-600 transition shadow-sm">
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-bold text-gray-800">
                                    <?php if($pendaftaran->pelatihanBerjalan && $pendaftaran->pelatihanBerjalan->tanggal_pelatihan): ?>
                                        <?php echo e(\Carbon\Carbon::parse($pendaftaran->pelatihanBerjalan->tanggal_pelatihan)->format('d M Y')); ?>
            ...
            <form action="<?php echo e(route('portal.pendaftaran.revisi', $pendaftaran->id)); ?>" method="POST" enctype="multipart/form-data">
                <?php echo csrf_field(); ?>
                <div class="space-y-4 pb-8">
                    ...
                    <?php $__currentLoopData = $dokMap; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field => $namaDoc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <?php if(!$fileExist): ?>
                            <?php if(in_array($field, ['sk', 'laporan', 'sop'])): ?>
                                ...
                                <div class="bg-white p-5 rounded-2xl border-2 border-gray-200 shadow-sm relative overflow-hidden">
                                    ...
                                    <div class="flex items-center justify-between mb-4 pl-1">
                                        <div class="flex items-center">
                                            ...
                                            <div>
                                                <p class="text-sm font-bold text-gray-800"><?php echo e($namaDoc); ?></p>
                                    ...
                                    <div id="upload-area-<?php echo e($field); ?>" class="ml-1">
                                        <label class="flex items-center justify-center w-full p-3 border border-dashed border-gray-300 rounded-xl cursor-pointer bg-gray-50 hover:bg-blue-50 hover:border-blue-400 transition-colors">
                                            ...
                                            <input type="file" name="file_<?php echo e($field); ?>" accept=".pdf,.jpg,.jpeg,.png" onchange="prosesRevisi(this, '<?php echo e($field); ?>')" />
                                        </label>
                                    </div>

                                    <div id="action-area-<?php echo e($field); ?>" class="hidden items-center justify-between bg-blue-50 p-3 rounded-xl border border-blue-100 ml-1 mt-2">
                                        <p class="text-xs text-blue-700 font-medium truncate mr-3 flex-1" id="nama-file-<?php echo e($field); ?>">namafile.jpg</p>
                                    </div>
                                </div>
                            <?php else: ?>
                                ...
                                <div class="bg-white p-5 rounded-2xl border-2 border-red-200 shadow-sm relative overflow-hidden">
                                    ...
                                    <div class="flex items-center justify-between mb-4 pl-1">
                                        <div class="flex items-center">
                                            ...
                                            <div>
                                                <p class="text-sm font-bold text-gray-800"><?php echo e($namaDoc); ?></p>
                                                <span class="bg-red-100 text-red-700 px-2 py-0.5 rounded text-[10px] font-bold mt-1 inline-block">Belum Diunggah (Wajib)</span>
                                            </div>
                                        </div>
                                    </div>
                                    
                                    <div class="bg-red-50 p-3 rounded-xl border border-red-100 mb-4 ml-1">
                                        <p class="text-xs font-bold text-red-800 mb-1">Perhatian:</p>
                                        <p class="text-xs text-red-600">Dokumen wajib ini belum Anda unggah. Harap segera lengkapi.</p>
                                    </div>

                                    <div id="upload-area-<?php echo e($field); ?>" class="ml-1">
                                        <label class="flex items-center justify-center w-full p-3 border border-dashed border-gray-300 rounded-xl cursor-pointer bg-gray-50 hover:bg-blue-50 hover:border-blue-400 transition-colors">
                                            <svg class="w-5 h-5 text-gray-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                                            <span class="text-sm text-gray-500 font-medium">Pilih file untuk melengkapi...</span>
                                            <input type="file" name="file_<?php echo e($field); ?>" accept=".pdf,.jpg,.jpeg,.png" onchange="prosesRevisi(this, '<?php echo e($field); ?>')" />
                                        </label>
                                    </div>
                        ...
                        <?php elseif($statusDoc == 'reject'): ?>
                            ...
                            <div class="bg-white p-5 rounded-2xl border-2 border-red-200 shadow-sm relative overflow-hidden">
                                ...
                                <div class="bg-red-50 p-3 rounded-xl border border-red-100 mb-4 ml-1">
                                    ...
                                    <p class="text-xs text-red-600 italic">"<?php echo e($catatanDoc ?? 'Silakan unggah ulang dokumen yang sesuai.'); ?>"</p>
                                </div>
                    ...
                    <button type="submit" id="btn-submit-revisi" class="w-full bg-blue-600 text-white font-bold text-lg py-4 rounded-2xl shadow-lg shadow-blue-600/30 hover:bg-blue-700 active:scale-95 transition-all mt-6 <?php echo e($pendaftaran->status != 'revisi' ? 'hidden' : ''); ?>">
                        <?php echo e($pendaftaran->status == 'revisi' ? 'Kirim Ulang Dokumen Revisi' : 'Kirim Dokumen Susulan'); ?>
...
</html><?php /**PATH C:\laragon\www\dashboard-mkt\resources\views/portal/cek-status.blade.php ENDPATH**/ ?>