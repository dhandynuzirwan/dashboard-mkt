...
<body class="bg-gray-50 text-gray-800 font-sans antialiased flex flex-col min-h-screen">
    ...
    <nav class="bg-white/80 backdrop-blur-md shadow-sm py-4 sticky top-0 z-30 border-b border-gray-100">
        <div class="max-w-4xl mx-auto px-5 flex items-center">
            <a href="<?php echo e(url('portal')); ?>" class="mr-4 p-2 bg-gray-50 rounded-full text-gray-500 hover:text-emerald-600 hover:bg-emerald-50 transition-colors active:scale-95">
    ...
    <main class="flex-grow px-5 py-8 w-full max-w-4xl mx-auto">
        ...
        <div id="login-section" class="w-full max-w-md mx-auto fade-in <?php echo e($kolektif ? 'hidden' : ''); ?>">
            ...
            <div class="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100">
                ...
                <form action="<?php echo e(route('portal.cek-status-perusahaan')); ?>" method="GET" class="space-y-6">
                    ...
                    <div id="form-login-id" class="space-y-6">
                        <div class="space-y-2">
                            ...
                            <div class="input-focus-ring transition-shadow rounded-xl relative">
                                ...
                                <input type="text" name="id_kolektif" class="w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-emerald-500 outline-none transition-colors text-gray-800 font-mono font-bold tracking-widest uppercase" placeholder="CORP-XXXX-XXX" value="<?php echo e(request('id_kolektif')); ?>" required>
                            </div>
                        </div>
                    </div>
        ...
        <div id="dashboard-section" class="w-full <?php echo e($kolektif ? '' : 'hidden'); ?>">
            ...
            <?php if($kolektif): ?>
                <div class="flex justify-between items-center mb-4 bg-emerald-600 p-5 rounded-3xl text-white shadow-lg">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h2 class="text-xl font-bold"><?php echo e($kolektif->perusahaan); ?></h2>
                            <p class="text-xs text-emerald-100 font-medium">ID: <?php echo e($kolektif->id_pendaftaran); ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-3 mb-8">
                    <div class="bg-white p-4 rounded-2xl shadow-sm border text-center">
                        <h3 class="text-2xl font-bold text-gray-800"><?php echo e($kolektif->pesertas->count()); ?></h3>
                        <p class="text-[10px] uppercase font-bold text-gray-500">Total Karyawan</p>
                    </div>
                    <div class="bg-white p-4 rounded-2xl shadow-sm border text-center">
                        <h3 class="text-2xl font-bold text-green-600"><?php echo e($kolektif->pesertas->whereIn('status', ['approve', 'diterima'])->count()); ?></h3>
                    ...
                    <div class="bg-red-50 p-4 rounded-2xl border border-red-100 text-center">
                        <h3 class="text-2xl font-bold text-red-600"><?php echo e($kolektif->pesertas->where('status', 'revisi')->count()); ?></h3>
                ...
                <div class="mt-8">
                    ...
                    <?php if($kolektif->pesertas->where('status', 'revisi')->count() > 0): ?>
                    ...
                    <div class="space-y-3">
                        <?php $__empty_1 = true; $__currentLoopData = $kolektif->pesertas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $peserta): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <?php
                            $pb = $peserta->pelatihanBerjalan;
                        ?>
                        <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col mb-3 emp-item" data-status="<?php echo e($peserta->status); ?>" data-program="<?php echo e($peserta->training->nama_training ?? ''); ?>">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                                    <h4 class="font-bold text-gray-800 emp-name"><?php echo e($peserta->nama_lengkap); ?></h4>
                                    <p class="text-xs text-gray-500"><?php echo e($peserta->training->nama_training ?? 'Belum ditentukan'); ?></p>
                                </div>
                                <div>
                                    <?php if(in_array($peserta->status, ['approve', 'diterima'])): ?>
                                        <span class="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-bold">Terverifikasi</span>
                                    <?php elseif($peserta->status == 'revisi'): ?>
                            ...
                            <?php if(in_array($peserta->status, ['approve', 'diterima', 'revisi'])): ?>
                                <div class="mt-2 pt-3 border-t border-gray-50">
                                    
                                    <?php if($peserta->status == 'revisi'): ?>
                                        <div class="mb-3 bg-red-50 p-3 rounded-xl border border-red-100">
                                            ...
                                            <ul class="text-[11px] text-red-600 space-y-1 list-disc list-inside">
                                                ...
                                                ?>
                                                <?php $__currentLoopData = $dokMap; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <?php if($peserta->{'status_'.$field} == 'reject'): ?>
                                                        <li><b><?php echo e($label); ?>:</b> <?php echo e($peserta->{'catatan_'.$field} ?? 'Perbaiki dokumen ini'); ?></li>
                                                    <?php endif; ?>
                                    ...
                                    <?php if($pb): ?>
                                        <p class="text-[11px] font-bold text-gray-500 mb-2"><i class="fas fa-calendar-alt text-indigo-400 mr-1"></i> Jadwal: <span class="text-gray-700"><?php echo e($pb->tanggal_pelatihan ? \Carbon\Carbon::parse($pb->tanggal_pelatihan)->format('d M Y') : 'Menunggu Penjadwalan'); ?></span></p>
                                        <div class="flex flex-wrap gap-2">
                                            <?php if($pb->link_zoom_pelatihan): ?>
                                            <a href="<?php echo e($pb->link_zoom_pelatihan); ?>" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors">
                                                <i class="fas fa-video mr-1"></i> Zoom
                                            </a>
                                            <?php endif; ?>
                                            <?php if($pb->link_zoom_asesmen): ?>
                                            <a href="<?php echo e($pb->link_zoom_asesmen); ?>" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-purple-50 hover:bg-purple-100 text-purple-600 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors">
                                                <i class="fas fa-video mr-1"></i> Asesmen
                                            </a>
                                            <?php endif; ?>
                                            <?php if($pb->modul): ?>
                                                <?php if($peserta->is_modul_downloaded): ?>
                                                ...
                                                <?php else: ?>
                                                <a href="<?php echo e(route('portal.download-modul', $peserta->id)); ?>" onclick="return confirm('PERINGATAN!\n\nSesuai dengan SOP dan Hak Cipta, Modul Materi untuk Peserta HANYA DAPAT DIUNDUH 1 KALI.\n\nApakah Anda yakin ingin mengunduhnya sekarang?');" class="inline-flex items-center px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors active:scale-95 shadow-sm">
                                                    <i class="fas fa-book mr-1"></i> Modul Materi
                                                </a>
                                                <?php endif; ?>
                                            <?php endif; ?>
                                            <?php if($pb->rundown_pelatihan): ?>
                                            <a href="<?php echo e(asset($pb->rundown_pelatihan)); ?>" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-green-50 hover:bg-green-100 text-green-700 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors">
                                                <i class="fas fa-list mr-1"></i> Rundown
                                            </a>
                                            <?php endif; ?>
                                            <?php if($pb->background_zoom): ?>
                                            <a href="<?php echo e(asset($pb->background_zoom)); ?>" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-teal-50 hover:bg-teal-100 text-teal-700 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors">
                                                <i class="fas fa-image mr-1"></i> BG Zoom
                                            </a>
                                            <?php endif; ?>
                                        </div>
                                        <?php if(!$pb->link_zoom_pelatihan && !$pb->link_zoom_asesmen && !$pb->modul): ?>
                    ...
                    <div class="fixed inset-0 z-10 overflow-y-auto">
                        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                            <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                ...
                                <!-- Modal Body -->
                                <form action="<?php echo e(route('portal.kolektif.revisi', $kolektif->id)); ?>" method="POST" enctype="multipart/form-data">
                                    <?php echo csrf_field(); ?>
                                    <div class="px-6 py-6">
...
<?php /**PATH C:\laragon\www\dashboard-mkt\resources\views/portal/cek-status-perusahaan.blade.php ENDPATH**/ ?>